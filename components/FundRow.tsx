      ...
      <div className="flex items-center py-2.5 px-2 bg-white border-b border-gray-100 text-sm hover:bg-gray-50 transition-colors">
        ...
        <div className="w-[28%] sm:w-[25%] flex flex-col justify-center overflow-hidden">
          <div className="flex items-center gap-1">
            <span className="text-[#2c68a8] font-medium text-[14px] sm:text-[15px] leading-tight truncate" title={fund.name}>{fund.name}</span>
            {onDelete && (
              <button
          ...
          <span className="text-[#2c68a8] text-[11px] sm:text-xs mt-0.5">{fund.code}</span>
        </div>

        {/* Column 2: Price (场内价格 for LOF funds, or previous close NAV for regular funds) */}
        <div className="w-[14%] sm:w-[16%] flex flex-col items-center justify-center text-center">
          {fund.valuation > 0 ? (
            <>
        ...
        <div className="w-[14%] sm:w-[16%] flex items-center justify-center text-center">
          {fund.limitText ? (
            <span className={`text-[11px] sm:text-xs px-1 sm:px-1.5 py-0.5 rounded ${
        ...
        <div className="w-[16%] sm:w-[11%] flex items-center justify-center gap-1">
          {onToggleMonitoring && (
            <>
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                />
                <div className="w-10 h-5.5 sm:w-11 sm:h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4.5 after:w-4.5 sm:after:h-5 sm:after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
              </label>
              {fund.monitoringEnabled && (
                <button
                  onClick={() => setShowTriggerSettings(!showTriggerSettings)}
                  className="text-blue-500 hover:text-blue-700 text-lg sm:text-xl font-medium min-w-[32px] min-h-[32px] flex items-center justify-center"
                  title="配置触发器"
                >
                  ⚙
                </button>
