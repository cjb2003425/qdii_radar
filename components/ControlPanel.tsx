...
export const ControlPanel: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-soft border border-slate-100 overflow-hidden mb-4">
      {/* Header Bar */}
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 px-3 md:px-5 py-2 md:py-3 flex justify-between items-center text-white relative overflow-hidden">
        {/* Decorative background element */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/10 rounded-full blur-3xl -mr-16 -mt-16 pointer-events-none"></div>

        <div className="flex items-center gap-2 relative z-10">
          <div className="bg-indigo-500/20 p-1 rounded-xl backdrop-blur-sm ring-1 ring-white/10">
             <Activity className="text-indigo-300 w-3.5 h-3.5" />
          </div>
          <div>
            <span className="font-bold text-sm md:text-[17px] tracking-tight block">监控中心</span>
            <span className="text-[9px] text-slate-400 font-medium hidden md:block leading-none mt-0.5">System Control</span>
          </div>
        </div>

        <div className="relative z-10 flex items-center gap-2 bg-slate-800/50 backdrop-blur-md border border-white/10 px-2 py-1 rounded-full">
          ...
          <span className="text-[9px] font-medium text-emerald-50">
      ...
      {/* Cards Area */}
      <div className="p-2.5 md:p-3 grid grid-cols-1 md:grid-cols-3 gap-2 bg-slate-50/50">
        {/* Card 1: Monitor Toggle */}
        <div className={`bg-white rounded-xl p-2 md:p-3 flex justify-between items-center border shadow-sm hover:shadow-md transition-all duration-300 ${
          isMonitoringEnabled
          ...
          <div className="flex items-center gap-2">
            <div className={`p-1.5 rounded-xl transition-colors ${
              ...
              <Eye size={16} className="md:w-[20px] md:h-[20px]" strokeWidth={1.5} />
            </div>
            <div>
          ...
          <button
            ...
            disabled={toggling}
            className={`w-10 h-5.5 md:w-10 md:h-6 rounded-full p-0.5 transition-all duration-300 ease-in-out cursor-pointer active:scale-95 flex-shrink-0 ${
              toggling ? 'opacity-50 cursor-not-allowed' : ''
            } ${
              isMonitoringEnabled ? 'bg-gradient-to-r from-green-500 to-emerald-600 shadow-inner' : 'bg-slate-300'
            }`}
          >
            <div className={`bg-white w-4.5 h-4.5 md:w-5 md:h-5 rounded-full shadow-sm transform transition-transform duration-300 ease-out ${
              isMonitoringEnabled ? 'translate-x-4.5' : 'translate-x-0'
            }`}></div>
          </button>
        </div>
        ...
        <div className={`bg-white rounded-xl p-2 md:p-3 flex justify-between items-center border shadow-sm hover:shadow-md transition-all duration-300 ${
          ...
          <div className="flex items-center gap-2">
            <div className={`p-1.5 rounded-xl transition-colors ${
              ...
              <Clock size={16} className="md:w-[20px] md:h-[20px]" strokeWidth={1.5} />
            </div>
            <div>
          ...
          >
            <div className={`bg-white w-4.5 h-4.5 md:w-5 md:h-5 rounded-full shadow-sm transform transition-transform duration-300 ease-out ${
              isTradingHours ? 'translate-x-4.5' : 'translate-x-0'
            }`}></div>
          </button>
        </div>
        ...
        <div className={`bg-white rounded-xl p-2 md:p-3 flex justify-between items-center border shadow-sm hover:shadow-md transition-all duration-300 ${
          ...
          <div className="flex items-center gap-2">
            <div className={`p-1.5 rounded-xl transition-colors ${
              ...
              <Hourglass size={16} className="md:w-[20px] md:h-[20px]" strokeWidth={1.5} />
            </div>
            <div>
              <div className="font-bold text-slate-700 text-xs md:text-[13px] mb-0.5">检查间隔</div>
              <div className="text-[9px] text-slate-500 font-medium">{intervalMinutes} 分钟</div>
            </div>
          </div>
          <div className={`relative flex-shrink-0`}>
            <select
              ...
              className={`appearance-none px-2 py-1 pr-6 rounded-lg border text-[11px] font-semibold shadow-sm transition-all cursor-pointer min-h-[36px] w-full ${
                updatingInterval
                  ? 'bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed'
                  : 'bg-white border-blue-200 text-blue-700 hover:bg-blue-50 hover:border-blue-300 active:bg-blue-100'
              }`}
            >
      ...
      {/* Warning Banner */}
      {!isMonitoringEnabled && (
        <div className="mx-2.5 mt-2 md:mx-3 relative overflow-hidden rounded-lg shadow-sm border border-amber-300 bg-gradient-to-br from-amber-50 to-yellow-50">
          <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-amber-200/50 to-transparent rounded-bl-full"></div>
          <div className="relative p-2 flex items-start gap-1.5">
      ...
      <div className="px-2.5 md:px-3 pb-2.5 md:pb-3 bg-slate-50/50">
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-2 md:p-3">
          <h3 className="text-[9px] md:text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 md:mb-2 flex items-center gap-1">
            <Activity size={10} /> System Health
          </h3>
          <div className="flex items-center justify-between bg-slate-50/50 rounded-lg p-1.5 md:p-2 border border-slate-100">
            <div className="flex items-center gap-1.5">
              <div className="bg-white p-0.5 rounded-md text-emerald-500 shadow-sm border border-slate-100">
                <Clock size={12} />
              </div>
              <div className="flex flex-col">
                <span className="text-[8px] text-slate-400 font-medium leading-none mb-0.5">检查</span>
                <span className="text-[10px] font-bold text-slate-700 font-mono leading-none">{lastCheckTime}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5">
              <div className="flex flex-col items-end">
                <span className="text-[8px] text-slate-400 font-medium leading-none mb-0.5">通知</span>
                <span className="text-[10px] font-bold text-slate-700 font-mono leading-none">{lastEmailTime}</span>
              </div>
              <div className="bg-white p-0.5 rounded-md text-indigo-500 shadow-sm border border-slate-100">
                <Mail size={12} />
              </div>
            </div>
          </div>
      ...
      <div className="px-2.5 md:px-3 pb-2.5 md:pb-3 bg-slate-50/50">
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-2 md:p-3">
          <h3 className="text-[9px] md:text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 md:mb-2 flex items-center gap-1">
            <LayoutDashboard size={10} /> Market Overview
          </h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-1.5 md:gap-2">
            <div className="flex flex-col overflow-hidden">
              <span className="text-[8px] font-semibold text-slate-400 mb-0.5">纳斯达克</span>
              <div className="flex items-baseline gap-0.5">
                <span className="text-sm font-bold leading-none tracking-tight text-emerald-600">
                  {marketData ? marketData.nasdaq.value.toLocaleString('en-US', {maximumFractionDigits: 0}) : '...'}
                </span>
                {marketData && marketData.nasdaq.change !== 0 && (
                  <span className={`text-[10px] font-medium ${marketData.nasdaq.change > 0 ? 'text-red-500' : 'text-green-500'}`}>
            ...
            <div className="flex flex-col overflow-hidden">
              <span className="text-[8px] font-semibold text-slate-400 mb-0.5">标普500</span>
              <div className="flex items-baseline gap-0.5">
                <span className="text-sm font-bold leading-none tracking-tight text-blue-600">
                  {marketData ? marketData.sp500.value.toLocaleString('en-US', {maximumFractionDigits: 0}) : '...'}
                </span>
                {marketData && marketData.sp500.change !== 0 && (
                  <span className={`text-[10px] font-medium ${marketData.sp500.change > 0 ? 'text-red-500' : 'text-green-500'}`}>
            ...
            <div className="flex flex-col overflow-hidden">
              <span className="text-[8px] font-semibold text-slate-400 mb-0.5">平均溢价</span>
            ...
            <div className="flex flex-col overflow-hidden">
              <span className="text-[8px] font-semibold text-slate-400 mb-0.5">可交易</span>
              <span className="text-sm font-bold leading-none tracking-tight text-indigo-600">
