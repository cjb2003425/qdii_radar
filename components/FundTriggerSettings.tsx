...
const FundTriggerSettings: React.FC<Props> = ({ fund, onTriggerChange, onClose }) => {
  ...
  const [newTriggerType, setNewTriggerType] = useState<'premium_high' | 'premium_low' | 'limit_change'>('premium_high');
        ...
        <>
          ...
          {!showAddForm ? (
            ...
            <div className="p-3 bg-gray-50 rounded space-y-2">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  {newTriggerType === 'limit_change' ? '限额阈值 (元)' : '阈值 (%)'}
                </label>
                <input
                  type="number"
                  step={newTriggerType === 'limit_change' ? '1' : '0.1'}
                  value={newThreshold}
                  onChange={(e) => setNewThreshold(e.target.value)}
                  placeholder={newTriggerType === 'limit_change' ? '例如: 100000' : '例如: 5.0'}
                  className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
