...
export const fetchQDIIFunds = async (): Promise<FundData[]> => {
  ...
  // 1. Try Backend for limit data
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), API_CONFIG.REQUEST_TIMEOUT);
    
    // 将用户基金代码列表传给后端
    const userFundCodes = userFunds.map(f => f.code).join(',');
    const backendUrl = `${API_CONFIG.BACKEND_URL}?codes=${userFundCodes}`;
...
function mergeUserFundsWithBackendData(userFunds: ReturnType<typeof getUserFunds>, backendData: FundData[]): FundData[] {
  ...
  return userFunds.map(userFund => {
    const backendFund = backendMap.get(userFund.code);
    
    if (backendFund) {
      // 如果后端有数据，使用后端的净值和限额信息
      return {
        id: userFund.code,
        name: userFund.name,
        code: userFund.code,
